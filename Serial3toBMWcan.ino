...
void requestData() {
  if (doRequest){
    Serial.write("A"); // Send A to request real time data
    doRequest = false;
  }
  #ifdef REV1_5
...
void setup(){
  ...
  Serial3.println ("Version date: 8.5.2025"); // To see from debug serial when is used code created.
...
void SendDataToSpeeduino(){
  ...
  switch (CanAddress)
  {
    case 0x613:  // Odometer and fuel level
      Serial.write(1);                        // send 1 to confirm cmd received and valid
      Serial.write(canin_channel);            // confirms the destination channel
      Serial.write(odometerLSB);              // write back the requested data
      Serial.write(odometerMSB);
      Serial.write(FuelLevel);
      Serial.write(lowByte(runningClock));
      Serial.write(highByte(runningClock));
    ...
    case 0x615:  // Ambient temp
      ...
      Serial.write(ambientTemp);              // write back the requested data
        for (int i=0; i<4; i++) {
          Serial.write(0);
      }
    break;
    case  0x153:  // VSS
      ...
      Serial.write(canin_channel);            // confirms the destination channel
      Serial.write(0);
      Serial.write(lowByte(VSS));
      Serial.write(highByte(VSS));
      for (int i=0; i<5; i++) {
          Serial.write(0);
      }
    break;
    case  0x1F0:  // VSS for each invidual wheel
      Serial.write(1);                        // send 1 to confirm cmd received and valid
      Serial.write(canin_channel);            // confirms the destination channel               //write back the requested data
      Serial.write(lowByte(VSS1));
      Serial.write(highByte(VSS1));
      Serial.write(lowByte(VSS2));
      Serial.write(highByte(VSS2));
      Serial.write(lowByte(VSS3));
      Serial.write(highByte(VSS3));
      Serial.write(lowByte(VSS4));
      Serial.write(highByte(VSS4));
    break;
    default:
      ...
      Serial3.print ("Wrong CAN address");
    break;
  }
}
...
void displayData(){
  Serial3.print ("RPM-"); Serial3.print (currentStatus.RPM); Serial3.print("\t");
  Serial3.print ("PW-"); Serial3.print (currentStatus.PW1); Serial3.print("\t");
  Serial3.print ("PWcount-"); Serial3.print (PWcount); Serial3.print("\t");
  Serial3.print ("CLT-"); Serial3.print (CLT); Serial3.print("\t");
  Serial3.print ("TPS-"); Serial3.print (TPS); Serial3.println("\t");
...
void processData(){   // necessary conversion for the data before sending to CAN BUS
  ...
  else
  {
    ...
    Serial3.print ("Error. RPM Received:"); Serial3.print (currentStatus.RPM); Serial3.print("\t");
  }
  ...
  {
    data_error = true;  // data received is probaply corrupted, don't use it.
    Serial3.print ("Error. CLT received:"); Serial3.print (currentStatus.CLT); Serial3.print("\t");
  }
  ...
  {
    data_error = true; // data received is probaply corrupted, don't use it.
    Serial3.print ("Error. TPS received:"); Serial3.print (currentStatus.TPS); Serial3.print("\t");
  }
}
...
void ReadSerial()
{
  ...
  switch (currentCommand)
  {
    ...
    break;
    default:
      Serial3.print ("Not an A or R message ");
      Serial3.println (currentCommand);
    break;
  }
}
...
void loop() {
  switch(SerialState) {
    case NOTHING_RECEIVED:
      ...
      break;
    case A_MESSAGE:
      if (Serial.available() >= 74) { HandleA(); }  // read and process the A-message from serial3, when it's fully received.
      break;
    case R_MESSAGE:
      if (Serial.available() >= 3) {  HandleR(); }  // read and process the R-message from serial3, when it's fully received.
      break;
    case PWM_MESSAGE:
      if (Serial.available() >= 2) {  Handle_r(); }  // read and process the r-message from serial3, when it's fully received.
      break;
    default:
      break;
  }
  ...
  if ( (millis()-oldtime) > 500) { // timeout if for some reason reading serial3 fails
    oldtime = millis();
    Serial3.println ("Timeout from speeduino!");
    doRequest = true;                // restart data reading
  }
